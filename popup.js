...
function minuteSliderHTML(id, value, label, disabled) {
  const minutes = Math.min(60, Math.max(1, Number(value) || 10));
  return `<div class="notification-range"><input id="${id}" type="range" min="1" max="60" step="1" value="${minutes}" aria-label="${label}" ${disabled ? "disabled" : ""}/><output for="${id}"><span data-minute-output="${id}">${minutes}</span><small>min</small></output></div>`;
}
...
function wireSettings() {
  ...
  if (rm) rm.addEventListener("input", (e) => {
    ...
    if (output) output.textContent = state.reminderMinutes;
    const summary = document.querySelector(".notification-confirmation p");
    if (summary) summary.textContent = notificationSummary();
  });
  ...
  if (im) im.addEventListener("input", (e) => {
    ...
    if (output) output.textContent = state.iqamaMinutes;
    const summary = document.querySelector(".notification-confirmation p");
    if (summary) summary.textContent = notificationSummary();
  });
