...
function persistNotificationKeys() {
  ...
  try {
    fs.writeFileSync(global.settingsPath, JSON.stringify(global.settingsData));
  } catch (error) {
...
function getRandomNotificationMessage(prayerName, isPre = false) {
  ...
  function runNotificationScheduler() {
    ...
    const now = new Date();
    const events = dueEvents(prayerSchedule, reminderSettings, now);
    for (const event of events) {
      if (sentNotificationKeys.has(event.key)) continue;
      sentNotificationKeys.add(event.key);
      const msg = event.type === 'pre'
        ? getRandomNotificationMessage(event.prayer, true)
        : getRandomNotificationMessage(event.prayer, false);
      const title = event.type === 'pre'
        ? `${msg.title} (بعد ${event.minutes} دقيقة)`
        : event.type === 'iqama'
          ? `إقامة صلاة ${event.prayer}`
          : msg.title;
      const body = event.type === 'iqama'
        ? `حان وقت الإقامة — بعد ${event.minutes} دقيقة من الأذان.`
        : msg.body;
    ...
    if (events.length) persistNotificationKeys();
  }

  // A short interval plus a delivery window makes delayed wake-ups safe while
  // the persisted keys prevent duplicate notifications after repeated ticks.
...
app.on('window-all-closed', () => {
  // The tray process remains alive so prayer notifications continue after the
  // window is closed. Use the tray menu's Quit action to terminate the app.
});
